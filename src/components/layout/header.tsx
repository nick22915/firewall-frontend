...
import { Menu as MenuIcon, X as XIcon, ChevronDown, Shield } from "lucide-react";
...
import { navigationItems, NavItem, NavLink } from "@/lib/navigation";
...
export function Header() {
      ...
      {navigationItems.map((item) => {
        ...
        return (
          <DropdownMenu key={item.label}>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                className={cn(
                  ...
                  isParentActive ? "bg-primary/70 text-primary-foreground" : "text-primary-foreground"
                )}
              >
            ...
            <DropdownMenuContent align="start" className="w-56 bg-card text-card-foreground shadow-lg rounded-md">
  ...
  const mobileNav = (
    <div className="md:hidden">
      <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
        ...
        <SheetContent side="left" className="w-full max-w-xs bg-background p-0 sm:max-w-sm">
  ...
  if (!isMounted) {
    return ( // Skeleton or minimal header during SSR/hydration mismatch prevention phase
      <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-primary shadow-sm">
        <div className="container mx-auto flex h-16 items-center justify-between px-4 md:px-6">
          <Link href="/" passHref >
            
              <Shield className="h-7 w-7" />
              <span>Dashboard Navigator</span>
